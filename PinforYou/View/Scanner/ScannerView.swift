    ...
    func metadataOutput(_ output: AVCaptureMetadataOutput, didOutput metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {
        ...
        if let metadataObject = metadataObjects.first as? AVMetadataMachineReadableCodeObject,
            ...
            if let stringValue = qrCodeObject.stringValue {
                ...
                var hashed0 = stringValue.split(separator: ",")[0]
                var hashed1 = hashed0.split(separator: ":")[1]
                var hashed2 = hashed1.split(separator: "\"")[0]
                ...
                displayAlertTimer = Timer.scheduledTimer(withTimeInterval: 2.0, repeats: false) { [weak self] _ in
                    ...
                    if let money = self?.inputMoney.text?.split(separator: " ")[0] {
                        ...
                        NotificationCenter.default.post(name: NSNotification.Name("hashed"), object: hashed2, userInfo: nil)
...
struct ScannerMainView : View {
    ...
    @State var selectedCategory = "편의점"
    @State var storeName = ""
    ...
    let hashedNotification = NotificationCenter.default
        .publisher(for: NSNotification.Name("hashed"))
    ...
    var body: some View {
        if scannerViewModel.isFinshed {
            ...
            VStack {
                ...
                TextField("가게명 입력", text: $storeName)
                
                
                QRScannerView(scannedData: $scannedData)
                    .onReceive(scannedDataNotification.zip(scannedCardidNotification, hashedNotification)) { (Data, cardID, id) in
                        scannedData = Data.object as? String ?? ""
                        cardid = cardID.object as? Int ?? 0
                        hashedid = id.object as? String ?? ""
                        
                        let acategory = convertToCategory(from: selectedCategory)
                        
                        scannerViewModel.send(action: .storePayment, amount: Int(scannedData)!, cardid: cardid, hashed: hashedid, storeName: storeName, category: acategory)
                    }
